...
<header class="custom-navbar navbar navbar-light bg-white p-0 align-items-stretch">
    <a class="navbar-brand menu-width container-md bg-semi-dark bg-white text-center" href="{{ admin_url('/') }}">
        <span class="short text-black">{!! config('admin.logo-mini', config('admin.name')) !!}</span><span class="long text-black">{!! config('admin.logo', config('admin.name')) !!}</span>
    </a>
        ...
        <ul class="nav order-2 ms-auto d-flex align-items-center">
            ...
            <li class="nav-item">
                ...
                <ul class="dropdown-menu dropdown-menu-end user-menu" aria-labelledby="user-menu-link">
                    ...
                    <li class="user-header text-center bg-semi-dark bg-white p-3">
                        <span class="bg-light inline rounded-circle user-image medium">
                            <img src="{{ Admin::user()->avatar }}" alt="User Image">
                        </span>
                        <p>
                            <h2 class="text-black">{{ Admin::user()->name }}</h2>
                            <small class="text-black">Member since admin {{ Admin::user()->created_at }}</small>
                        </p>
                    </li>
                    <li class="user-footer p-2 clearfix bg-light">
                        <div class="float-start">
                            <a href="{{ admin_url('auth/setting') }}" class="btn btn-info">{{ __('admin.setting') }}</a>
                        </div>
                        <div class="float-end">
                            <a href="{{ admin_url('auth/logout') }}" class="btn no-ajax btn-danger">{{ __('admin.logout') }}</a>
                        </div>
                    </li>
                </ul>
            </li>
